...
st.markdown(
    ...
    /* V7.1 · Estado para entrenar / Entrenador */
    .rcp-readiness {
        border: 1px solid var(--rcp-border);
        border-radius: 18px;
        padding: .9rem 1rem;
...
DEVELOPMENT_FOCUS_LABELS = {
    ...
    "UMBRAL": "🔥 Ritmo sostenido / umbral",
    "VELOCIDAD_ECONOMIA": "⚡ Velocidad y eficiencia al correr",
    "ESPECIFICO_CARRERA": "🎯 Preparación específica para la carrera",
...
def ui_status_label(value):
    """Traduce estados internos a lenguaje visible sin modificar los códigos guardados."""
    raw = str(value or "").upper().strip()
    mapping = {
        "ACTIVE": "Activo", "FUTURE": "Futuro", "ARCHIVED": "Archivado",
        "COMPLETED": "Completado", "COMPLETADO": "Completado",
        "MODIFICADO": "Modificado", "OMITIDO": "Omitido", "PENDIENTE": "Pendiente",
        "PENDING": "Pendiente", "PREVIEW": "Vista previa", "APPLIED": "Aplicado",
        "REVERTED": "Revertido", "FAILED": "No aplicado", "BASELINE": "Plan original",
        "PROTECTED": "Protegido", "REDUCED": "Reducido", "RESTORED": "Restaurado",
        "RESCHEDULED_QUALITY": "Sesión de calidad reubicada",
        "RESCHEDULED_LONG": "Tirada larga reubicada",
        "INSERTED_REPLAN": "Sesión añadida por reorganización",
        "PROTECT_WEEK": "Semana protegida", "RETURN_WEEK": "Semana de retorno",
        "REBUILD_WEEK": "Semana reconstruida", "REBUILT_WEEK": "Semana reconstruida",
        "GREEN": "Normal", "YELLOW": "Cautela", "ORANGE": "Reducir", "RED": "No intensidad",
    }
    if raw in mapping:
        return mapping[raw]
...
def ui_test_context_label(value):
    return "Prueba" if str(value) == "Test" else str(value)


def ui_phase_label(value):
    return {
        "BASE": "Base aeróbica",
        "DESARROLLO": "Desarrollo",
        "ESPECÍFICA": "Preparación específica",
        "TAPER": "Puesta a punto",
        "CARRERA": "Carrera",
    }.get(str(value or "").upper(), str(value or "—").replace("_", " ").title())
...
# ============================================================
# V8.0.1 · MODO DE PRUEBAS EN MEMORIA
# ============================================================
...
def _qa_scenarios():
    return {
        "✅ Semana normal": {"adherence": 100, "readiness": 84, "pain": 1, "fatigue": 2.1, "rpe_excess": 0.2, "missed": 0, "illness": False, "gait": False, "trend_pct": 1.0, "load": 1380, "prior_load": 1320, "days_off": 0, "expected": "PROGRESAR", "replan": "Sin reorganización del plan necesaria"},
        ...
        "🚀 Mejora aeróbica": {"adherence": 100, "readiness": 87, "pain": 0, "fatigue": 2.0, "rpe_excess": -0.4, "missed": 0, "illness": False, "gait": False, "trend_pct": 3.1, "load": 1430, "prior_load": 1370, "days_off": 0, "expected": "PROGRESAR", "replan": "Sin reorganización del plan necesaria"},
        "📉 Posible meseta": {"adherence": 98, "readiness": 79, "pain": 1, "fatigue": 2.6, "rpe_excess": 0.3, "missed": 0, "illness": False, "gait": False, "trend_pct": 0.1, "load": 1390, "prior_load": 1380, "days_off": 0, "expected": "MANTENER", "replan": "Mantener y considerar Prueba RCP"},
...
def _qa_calibration_samples():
    return [
        {"Velocidad": 10.5, "Ritmo": "5:43/km", "RPE": 3}, {"Velocidad": 10.9, "Ritmo": "5:30/km", "RPE": 3},
        {"Velocidad": 11.3, "Ritmo": "5:19/km", "RPE": 4}, {"Velocidad": 11.7, "Ritmo": "5:08/km", "RPE": 5},
        {"Velocidad": 12.0, "Ritmo": "5:00/km", "RPE": 6}, {"Velocidad": 11.1, "Ritmo": "5:24/km", "RPE": 4},
    ]
...
def _qa_automated_matrix():
    ...
    for name, m in _qa_scenarios().items():
        ...
        rows.append({"Escenario": name, "Esperado": m.get("expected"), "Obtenido": got, "Resultado": "CORRECTO" if got == m.get("expected") else "ERROR"})
    zones = _qa_zones_from_benchmark()
    easy = sum(zones["Rodaje fácil"]) / 2
    threshold = sum(zones["Tempo / umbral"]) / 2
    rows.append({"Escenario": "🎯 Zonas", "Esperado": "Umbral más rápido", "Obtenido": "OK" if threshold < easy else "ERROR", "Resultado": "CORRECTO" if threshold < easy else "ERROR"})
    plan = _qa_plan_rows(); weekly = {}
    for r in plan: weekly[r["Semana"]] = weekly.get(r["Semana"], 0) + r["KM"]
    safe_jump = weekly[2] <= weekly[1] * 1.12
    rows.append({"Escenario": "🛡️ Progresión semanal", "Esperado": "≤12%", "Obtenido": f"{((weekly[2]/weekly[1])-1)*100:.1f}%", "Resultado": "CORRECTO" if safe_jump else "ERROR"})
    return rows


def render_qa_sandbox():
    """Modo de pruebas local: no consulta ni escribe filas de usuario en Supabase."""
    st.markdown(
        f"""<div class="rcp-hero"><div class="rcp-eyebrow">RUNNINGCOACHPRO · V{APP_VERSION} · MODO DE PRUEBAS</div>
        <h2>🧪 Corredor ficticio · memoria temporal</h2>
        <p>Entorno de pruebas sin cuenta real. Ningún botón de este modo escribe perfiles, planes, registros, estado para entrenar, pruebas ni revisiones en Supabase.</p>
    ...
    top1.info("Los datos desaparecen al salir o reiniciar el modo de pruebas. Úsalo para simular situaciones sin alterar una cuenta real.")
    if top2.button("🚪 Salir del modo de pruebas", use_container_width=True):
        st.session_state.pop("rcp_qa_mode", None); st.session_state.pop("rcp_qa_applied", None); st.rerun()

    tabs = st.tabs(["🎛️ Escenarios", "🎯 Zonas", "🎚️ Ritmo y esfuerzo", "🗓️ Plan", "✅ Batería de pruebas"])
    with tabs[0]:
        ...
        with st.expander("Ajustar métricas manualmente", expanded=False):
            ...
            m["adherence"] = c1.slider("Adherencia %", 0, 100, int(m["adherence"])); m["readiness"] = c2.slider("Estado para entrenar", 0, 100, int(m["readiness"])); m["pain"] = c3.slider("Dolor post máx.", 0, 10, int(m["pain"])); m["fatigue"] = c4.slider("Fatiga post media", 1.0, 5.0, float(m["fatigue"]), 0.1)
            c5, c6, c7 = st.columns(3)
            m["rpe_excess"] = c5.slider("Esfuerzo por encima de lo previsto", -2.0, 3.0, float(m["rpe_excess"]), 0.1); m["missed"] = c6.slider("Sesiones omitidas", 0, 4, int(m["missed"])); m["days_off"] = c7.slider("Días sin entrenar", 0, 21, int(m["days_off"]))
        ...
        k1, k2, k3, k4 = st.columns(4); k1.metric("Adherencia", f"{m['adherence']}%"); k2.metric("Estado para entrenar", f"{m['readiness']}/100"); k3.metric("Dolor", f"{m['pain']}/10"); k4.metric("Carga por esfuerzo", f"{int(m['load'])}")
        ...
        next_km = _qa_next_week_km(decision); n1, n2, n3 = st.columns(3); n1.metric("Semana actual", "35.0 km"); n2.metric("Semana 2 original", "36.4 km"); n3.metric("Propuesta de prueba", f"{next_km:.1f} km", delta=f"{next_km-36.4:+.1f} vs plan original")
        st.caption(f"Reorganización del plan: **{m.get('replan')}**")
        trend = m.get("trend_pct")
        ...
        if selected == "📉 Posible meseta": st.info("🧪 Prueba RCP sugerida: **5K controlado** en una ventana de 7–14 días, siempre que recuperación siga estable.")
        elif selected in {"🦵 Dolor alto", "🤒 Enfermedad aguda", "🥵 Fatiga acumulada"}: st.caption("Prueba RCP: no recomendada mientras recuperación esté comprometida.")
        else: st.caption("Prueba RCP: no imprescindible ahora; la referencia de rendimiento ficticia reciente sigue siendo utilizable.")
        if st.button("🧪 Aplicar decisión SOLO en el modo de pruebas", type="primary", use_container_width=True):
    ...
    with tabs[1]:
        st.subheader("🎯 Ritmos y zonas del corredor ficticio"); use_test = st.toggle("Simular nueva Prueba RCP de 5K en 23:30", value=False, key="qa_test_toggle")
        if use_test: zones = _qa_zones_from_benchmark(5.0, 23*60+30); source = "Prueba RCP 5K · 23:30"
        else: zones = _qa_zones_from_benchmark(10.0, 50*60); source = "Marca reciente 10K · 50:00"
        st.caption(f"Referencia de rendimiento activa: **{source}**. La prueba recalibra los ritmos, no incrementa kilómetros automáticamente.")
        rows = []
        for name, paces in zones.items():
            lo, hi = sorted([float(x) for x in paces]); rows.append({"Zona": name, "Ritmo": f"{_qa_fmt_pace(lo).replace('/km','')}–{_qa_fmt_pace(hi)}", "Caminadora": f"{_qa_speed(hi):.1f}–{_qa_speed(lo):.1f} km/h"})
        st.dataframe(rows, use_container_width=True, hide_index=True)
        st.warning("Estas zonas son referencias de entrenamiento, no umbrales fisiológicos medidos en laboratorio. El RPE (esfuerzo percibido) y la prueba del habla siguen siendo el control del esfuerzo.")

    with tabs[2]:
        st.subheader("🎚️ Aprendizaje ficticio de ritmo y esfuerzo"); st.caption("Ejemplo de cómo RCP aprende la relación velocidad ↔ esfuerzo con sesiones reales continuas."); samples = _qa_calibration_samples(); st.dataframe(samples, use_container_width=True, hide_index=True)
        ...
        p1, p2, p3 = st.columns(3); p1.metric("Muestras", n); p2.metric("Velocidad estimada a esfuerzo 3/10", f"{pred3:.1f} km/h"); p3.metric("Velocidad estimada a esfuerzo 4/10", f"{pred4:.1f} km/h")
        st.success("Con ≥6 observaciones ficticias, el modo de pruebas comprueba que el modelo responde a la historia personal y no solo a una tabla genérica.")
...
def extra_sessions_storage_ready():
    """V8.1.5: sesiones adicionales manuales, separadas del plan base."""
    try:
        (
            client.table("rc_extra_sessions")
            .select("id,user_id,plan_id,session_date,workout_type,planned_minutes,planned_km,target_rpe")
            .eq("user_id", USER_ID)
            .limit(1)
            .execute()
        )
...
def get_extra_sessions(plan_id=None, start_date=None, end_date=None):
    """Obtiene entrenamientos adicionales del plan activo sin convertirlos en sesiones del plan."""
    if not extra_sessions_storage_ready():
        return []
    active = get_active_plan_record() if plan_id is None else {"id": plan_id}
    ...
    try:
        q = (
            ...
            .select("*")
            .eq("user_id", USER_ID)
            .eq("plan_id", int(active["id"]))
            .order("session_date")
        )
        if start_date:
            q = q.gte("session_date", str(start_date))
        if end_date:
            q = q.lte("session_date", str(end_date))
        return q.execute().data or []
    except Exception:
        return []


def save_extra_session(payload):
    """Crea/edita una sesión adicional. Nunca escribe en rc_plan_sessions."""
    payload = dict(payload or {})
    active = get_active_plan_record()
    if not active or not active.get("id"):
        raise RuntimeError("No existe un plan activo para asociar el entrenamiento adicional.")
    session_date = str(payload.get("session_date") or "")
    if not session_date:
        raise ValueError("Falta la fecha del entrenamiento adicional.")
    payload["user_id"] = USER_ID
    payload["plan_id"] = int(active["id"])
    payload["updated_at"] = datetime.now(timezone.utc).isoformat()
    client.table("rc_extra_sessions").upsert(
        payload, on_conflict="user_id,plan_id,session_date"
    ).execute()


def delete_extra_session(extra_id, session_date=None, delete_training_log=True):
    """Elimina solo el adicional. Opcionalmente limpia su registro real del mismo día."""
    if extra_id is None:
        return
    client.table("rc_extra_sessions").delete().eq("user_id", USER_ID).eq("id", int(extra_id)).execute()
    if delete_training_log and session_date:
        # Solo se usa en días sin sesión del plan; así no toca un registro planificado.
        if str(session_date) not in globals().get("PLAN_BY_DATE", {}):
            try:
                delete_log(str(session_date))
            except Exception:
                pass


def extra_session_to_session(extra):
    """Adaptador para reutilizar RPE, carga y lenguaje de sesión sin alterar el plan."""
    extra = extra or {}
    label = str(extra.get("workout_type") or "Rodaje suave")
    upper = label.upper()
    if "RECUP" in upper:
        wtype = "Recuperación"
        wname = "Recuperación adicional"
    elif "CAMIN" in upper:
        wtype = "Rodaje"
        wname = "Caminata / trote adicional"
    else:
        wtype = "Rodaje"
        wname = "Rodaje suave adicional"
    km = float(extra.get("planned_km") or 0)
    minutes = int(extra.get("planned_minutes") or 0)
    target_rpe = int(extra.get("target_rpe") or 3)
    return {
        "id": None,
        "plan_id": extra.get("plan_id"),
        "session_date": extra.get("session_date"),
        "week_no": _week_no_for_date(parse_date_safe(extra.get("session_date")) or rcp_today(), 1)
            if "_week_no_for_date" in globals() else 1,
        "workout_type": wtype,
        "workout_name": wname,
        "planned_km": km,
        "target": f"RPE {target_rpe}/10 · {minutes} min",
        "intensity": f"RPE {target_rpe}/10",
        "description": str(extra.get("notes") or ""),
        "is_extra": True,
        "extra_session_id": extra.get("id"),
        "planned_minutes": minutes,
        "target_rpe": target_rpe,
    }


def extra_session_status_label(extra, log=None):
    log = log or {}
...
def pace_rpe_calibration_samples(surface="CAMINADORA", days=120):
    ...
    Para calibración continua excluye sesiones donde el ritmo medio de toda la sesión
    ...
    for log in logs:
        ...
        if not session:
            _extra = globals().get("EXTRA_BY_DATE", {}).get(str(log.get("session_date"))) or {}
            if _extra:
                session = extra_session_to_session(_extra)
...
def get_all_logs(limit=2000):
    """Historial longitudinal real del usuario, independiente del plan activo.

    Los cambios/recalibraciones de plan archivan el plan anterior; sus registros
    permanecen en rc_workout_logs con el plan_id histórico. Esta consulta deliberadamente
    NO filtra por plan_id para que el historial del corredor nunca desaparezca de Progreso.
    """
    try:
        return (
            client.table("rc_workout_logs")
...
def get_all_plan_sessions(limit=4000):
    """Sesiones de planes activos y archivados, solo para contextualizar historial."""
    try:
        return (
            client.table("rc_plan_sessions")
            .select("*")
...
def _dedupe_longitudinal_logs(rows):
    """Una actividad real por fecha para métricas longitudinales.

    Si una recalibración creó más de una versión de plan para una misma fecha, preferimos
    COMPLETADO/MODIFICADO sobre OMITIDO y, a igualdad, el registro actualizado más reciente.
    No borra nada: solo evita contar dos veces la misma actividad en estadísticas globales.
    """
    priority = {"COMPLETADO": 4, "MODIFICADO": 4, "OMITIDO": 2}
    chosen = {}
    for row in rows or []:
        ...
        score = (priority.get(status, 1), updated, int(row.get("id") or 0))
        previous = chosen.get(key)
        if previous is None or score > previous[0]:
            chosen[key] = (score, row)
    return [x[1] for x in sorted(chosen.values(), key=lambda item: str(item[1].get("session_date") or ""))]


def longitudinal_workout_logs():
    rows = globals().get("HISTORY_LOGS")
    if rows is not None:
        return rows
    return _dedupe_longitudinal_logs(get_all_logs())
...
def get_all_readiness_rows(limit=1000):
    """Historial de estado diario a través de todas las versiones de plan."""
    try:
        return (
            client.table("rc_daily_readiness")
            .select("*")
...
def longitudinal_readiness_rows():
    rows = globals().get("ALL_READINESS_ROWS")
    return rows if rows is not None else get_all_readiness_rows()
...
def save_readiness(payload, plan_id=None):
    ...
    if not pid:
        raise RuntimeError("No existe un plan activo para asociar el registro diario.")
    row = dict(payload)
    row["user_id"] = USER_ID
    row["plan_id"] = int(pid)
...
# ============================================================
# V7.2 · Persistencia de reorganización del plan
# ============================================================
...
def _session_zone_key(session):
    ...
    if "RECUP" in blob or "RUN_WALK" in blob:
        return "recovery"
    # La activación muestra la referencia de los tramos alegres, no el promedio del trote.
    if any(x in blob for x in ("ACTIVACION", "ACTIVACIÓN", "RECTAS", "SERIES", "INTERVAL", "VELOCIDAD", "CUESTA")):
        return "interval"
...
def _session_cycle_progress(session):
    """Progreso 0..1 del ciclo para graduar trabajo específico sin forzar la meta."""
    try:
        week_no = int((session or {}).get("week_no") or 0)
    except Exception:
        week_no = 0
    total_weeks = 0
    try:
        meta = (globals().get("ACTIVE_PLAN") or {}).get("metadata") or {}
        total_weeks = len(meta.get("weeks") or [])
    except Exception:
        total_weeks = 0
    if total_weeks <= 0:
        try:
            total_weeks = max(int(p.get("week_no") or 0) for p in (globals().get("PLAN") or []))
        except Exception:
            total_weeks = 0
    if week_no <= 0 or total_weeks <= 0:
        return 0.60
    return float(_clamp(week_no / max(1, total_weeks), 0.0, 1.0))


def _current_equivalent_goal_pace(assessment=None, goal_row=None):
    """Ritmo equivalente ACTUAL para la distancia objetivo desde test/marca disponible."""
    ...
    if test:
        try:
            eq = estimate_equivalent_time(float(test.get("duration_sec") or 0), float(test.get("distance_km") or 0), float(goal_km))
            if eq:
                return round(float(eq) / float(goal_km))
        except Exception:
            pass
    try:
        pp = v7_pace_profile(assessment, goal_row)
        if pp.get("race_current"):
            return round(float(pp["race_current"]))
    except Exception:
        pass
    return None


def _specific_session_paces(session, goal_row=None, assessment=None):
    """Referencia estrecha para bloques específicos: capacidad actual -> meta, progresivamente."""
    goal_row = goal_row or globals().get("ACTIVE_GOAL") or {}
    current = _current_equivalent_goal_pace(assessment, goal_row)
    target = None
    goal_km = GOAL_KM.get(str(goal_row.get("goal_type") or ""))
    try:
        if goal_km and goal_row.get("target_time_sec"):
            target = round(float(goal_row.get("target_time_sec")) / float(goal_km))
    except Exception:
        target = None
    center = _specific_race_training_pace(
        {"race_current": current, "race_target": target, "race": current or target},
        _session_cycle_progress(session),
    )
    if not center:
        return None
    # Rango estrecho para evitar falsa precisión. El RPE sigue siendo el control principal.
    return [max(1, int(center) - 3), int(center) + 6]
...
def treadmill_guidance(session, goal_row=None, assessment=None):
    ...
    La referencia secundaria nunca modifica la prescripción: manda el RPE (esfuerzo percibido) y prueba del habla.
    """
    session = session or {}
    goal_row = goal_row or globals().get("ACTIVE_GOAL") or {}
    ...
    if _zone_key == "specific":
        specific_paces = _specific_session_paces(session, goal_row, assessment)
        if specific_paces:
            return {
    ...
    # Zonas dinámicas: test reciente + marca + calibración personal.
    v8 = v8_dynamic_zones(assessment, goal_row)
    zone = (v8.get("zones") or {}).get(_zone_key)
    if zone:
        ...
        return {
            ...
            "note": "Referencia dinámica RCP V8. El RPE (esfuerzo percibido) y la prueba del habla siguen siendo el control principal del esfuerzo.",
        }
    ...
    return {
        ...
        "repeats": [],
        "source": "RPE",
        "note": "No hay una marca o meta temporal suficiente para asignar una velocidad responsable; usa el RPE (esfuerzo percibido) y prueba del habla.",
    }
...
def v8_dynamic_zones(assessment=None, goal_row=None):
    ...
    source = str(base.get("basis") or "RPE (esfuerzo percibido) y prueba del habla")
    benchmark_date = None
    ...
    if test:
        ...
        if test_zones:
            ...
            source = f"Prueba RCP {str(test.get('test_type') or '').upper()}"
            benchmark_date = parse_date_safe(test.get("test_date"))

    # Individualización solo en zonas donde un ritmo medio continuo representa el estímulo.
    personal_map = {
        "recovery": (2.0, 3.0),
        "easy": (3.0, 4.0),
        "long": (3.0, 4.0),
    ...
    if personal_used:
        source += f" + aprendizaje personal ({n_cal})"
...
def _zone_label(key):
    return {
        ...
        "threshold": "Ritmo fuerte controlado (umbral)",
        "interval": "Intervalos",
        "race": "Ritmo objetivo (meta)",
...
def v8_zone_rows(assessment=None, goal_row=None):
    ...
    for key in ("recovery", "easy", "long", "steady", "threshold", "interval", "race"):
        ...
        rows.append({
            ...
            "Ritmo": _pace_text_from_seconds(paces),
...
def v8_training_load_summary(days=56):
    ...
    entries = []
    for log in longitudinal_workout_logs():
...
def v8_recovery_snapshot():
    ...
    readiness = [r for r in longitudinal_readiness_rows() if (d := parse_date_safe(r.get("checkin_date"))) and start <= d <= today]
...
def v8_test_recommendation():
    ...
    if recovery["status"] in {"PROTEGER", "CAUTELA"}:
        return {"recommended": False, "reason": "Primero conviene estabilizar la recuperación antes de hacer una prueba de rendimiento."}
    race_date = parse_date_safe((ACTIVE_GOAL or {}).get("race_date"))
    if race_date and 0 <= (race_date - rcp_today()).days <= 14:
        return {"recommended": False, "reason": "La carrera objetivo está demasiado cerca para añadir una prueba específica."}
    ...
    if latest:
        ...
        if d and (rcp_today() - d).days < 35:
            return {"recommended": False, "reason": f"Ya existe una prueba válida de hace {(rcp_today()-d).days} días."}
    ...
    return {
        ...
        "reason": "Una prueba controlada puede actualizar tu referencia de rendimiento y recalibrar ritmos sin esperar una carrera oficial.",
    }
...
def _v81_history(days=210):
    ...
    for d,l in parsed_logs:
        ...
        if not sess:
            _extra = globals().get("EXTRA_BY_DATE", {}).get(str(l.get("session_date"))) or {}
            if _extra:
                sess = extra_session_to_session(_extra)
...
def v81_actionable_guidance(session, readiness_row, prediction):
    ...
    if _time_ctx.get("cap_minutes") and not _time_ctx.get("fits"):
        ...
        if kind in {"Series", "Tempo", "Específico"}:
            return {
    ...
    if decision == "REDUCIR":
        if kind in {"Series", "Tempo", "Específico", "Carrera"}:
...
def v8_self_checks():
    ...
    try:
        ...
        checks.append(("Límites de seguridad del plan operativos", v8_plan_guardrails().get("status") in {"OK","REVISAR"}))
...
def calculate_runner_score(answers):
    ...
    components = {
        ...
        "Experiencia en carreras o pruebas": race_score,
    }
...
def performance_summary(answers):
    ...
    if not has_mark:
        return {
            ...
            "usable": False,
            "message": "Los ritmos iniciales deberán apoyarse en RPE (esfuerzo percibido) y prueba del habla hasta disponer de una marca fiable.",
        }
    ...
    if not source_km or not source_seconds:
        return {
            ...
            "usable": False,
            "message": "Se declaró una carrera o prueba reciente, pero faltan distancia o tiempo válidos.",
        }
...
def session_time_context(session, day_value=None, assessment=None, goal_row=None, profile_row=None):
    ...
    try:
        ...
        pace_prof = {}
    kind = workout_kind(session)
    role = "LONG" if kind == "Larga" else ("QUALITY" if kind in ("Series", "Tempo", "Específico", "Carrera") else ("RECOVERY" if kind == "Recuperación" else "EASY"))
...
def _time_limited_session_fields(session, cap_minutes, assessment=None, goal_row=None):
    ...
    role = "LONG" if kind == "Larga" else ("QUALITY" if kind in ("Series", "Tempo", "Específico") else ("RECOVERY" if "RECUP" in str(session.get("workout_type") or "").upper() else "EASY"))
...
def v7_pace_profile(assessment, goal_row):
    ...
    perf = performance_summary(answers)
    out = {
        "basis": "RPE (esfuerzo percibido) y prueba del habla",
        "usable": False,
        "source": perf,
        "recovery": None,
        "easy": None,
    ...
    if str(perf.get("terrain") or "").lower() == "trail":
        out["basis"] = "RPE (esfuerzo percibido) y prueba del habla (marca trail no usada para ritmos de ruta)"
        return out
...
def build_v7_plan(goal_row, assessment, start_date_value=None):
    ...
    metadata = {
        ...
        "methodology": {
            ...
            "specific_pace": "Los bloques específicos usan una referencia progresiva desde la capacidad actual estimada por test/marca hacia la meta; el RPE manda.",
...
def can_generate_legacy_plan(goal_row, assessment):
    ...
    if goal_type not in GOAL_KM:
        return False, "Este objetivo será gestionado por el motor de planificación RCP; el generador anterior no tiene una plantilla válida para esta modalidad."
    ...
    if str(snapshot.get("base_status") or "") in ("BASE PREVIA", "INSUFICIENTE", "EVALUAR SEGURIDAD"):
        return False, (
            ...
            "en un plan específico; esta transición se resolverá con el motor de planificación RCP."
        )
...
def create_plan_record_for_goal(goal_row, base_profile, assessment, status="ACTIVE"):
    ...
    if reason or not rows:
        return None, reason or "El motor de planificación RCP no produjo sesiones."
...
def assessment_form(existing_assessment=None, onboarding=False):
    ...
    with st.form("rcp_assessment_form"):
        ...
        race_or_test = h3.checkbox(
            "Carrera o prueba reciente",
            value=bool(existing_answers.get("race_or_test_recent")),
            help="Una carrera o prueba reciente aporta información sobre tu experiencia competitiva, pero no define por sí sola tu nivel.",
        )

        st.markdown("#### Marca reciente · opcional")
        st.caption(
            "Si marcaste carrera o prueba reciente, completa estos datos. RunningCoachPro la usará como referencia de rendimiento; "
            "si no tienes una marca fiable, los ritmos futuros podrán iniciarse por RPE (esfuerzo percibido) y prueba del habla."
        )
        ...
        recent_mark_context = pm5.selectbox(
            ...
            format_func=ui_test_context_label,
        )
    ...
    if race_or_test:
        ...
        if not recent_km:
            st.error("Si declaras una carrera o prueba reciente, indica una distancia válida.")
            return False
        if not recent_seconds:
            st.error("Si declaras una carrera o prueba reciente, escribe un tiempo válido (MM:SS o HH:MM:SS).")
            return False
...
def official_goal_setup(profile, assessment):
    ...
    if plan_row:
        st.success("Objetivo guardado y plan personalizado creado. Tu historial queda disponible para futuras planificaciones.")
    else:
    ...
    if past:
        with st.expander("Historial de carreras registradas", expanded=False):
            for rd, race in reversed(past):
                log = HISTORY_LOG_BY_DATE.get(rd.isoformat()) if 'HISTORY_LOG_BY_DATE' in globals() else (LOG_BY_DATE.get(rd.isoformat()) if 'LOG_BY_DATE' in globals() else None)
...
def goal_management_ui(active_goal, active_plan, profile, assessment):
    ...
    if development_focus_storage_ready():
        with st.expander("⚙️ Cambiar enfoque de desarrollo", expanded=False):
            ...
            _choice = st.selectbox(
                ...
                help="Automático RCP es la opción recomendada: el motor elige el foco y aplica límites de seguridad según tu evaluación.",
    ...
    if active_plan and (engine_name.startswith("RCP-V7") or engine_name.startswith("RCP-V8")):
                ...
                if preview_reason:
                    ...
                    if _phase_rows:
                        st.dataframe([
                            {
                                "Semana": x.get("week"),
                                "Fase": ui_phase_label(x.get("phase")),
                    ...
                    st.caption("Primero se crea el nuevo plan y después se archiva el anterior. Tus entrenamientos registrados no se eliminan.")
        ...
        with st.expander("🧠 Actualizar mi plan con el motor actual", expanded=True):
            st.write(
            ...
            if preview_reason:
                st.warning(f"Todavía no puedo generar el plan: {preview_reason}")
            else:
                ...
                if st.button(
                    ...
                    if new_plan:
                        st.success("Plan actualizado. El plan anterior y sus registros permanecen en el historial.")
                        st.rerun()
                    else:
                        st.error(err or "No fue posible crear el plan.")
    ...
    with st.expander("✏️ Cambiar fecha o marca del objetivo actual"):
        ...
        if save_changes:
            ...
            if error:
                ...
                st.info("La revisión semanal y los ajustes adaptativos ya funcionan de forma separada. Cambiar la meta no borra ni reescribe tu historial.")
                st.rerun()
    ...
    for g in get_goals():
        goal_rows.append({
            ...
            "Estado": ui_status_label(g.get("status")),
        })
    if goal_rows:
    ...
    for p in get_plans():
        ...
        plan_rows.append({
            ...
            "Estado": ui_status_label(p.get("status")),
        })
    if plan_rows:
...
if not ASSESSMENT_READY:
    st.title("🏃 RunningCoachPro")
    st.error("Falta activar la Evaluación RCP en Supabase.")
    st.write(
...
if not PLANNING_READY:
    st.title("🏃 RunningCoachPro")
    st.error("Falta activar la estructura de Objetivos y Planes en Supabase.")
    st.write(
...
HISTORY_COMPLETED_LOGS = [
    x for x in HISTORY_LOGS
    if str(x.get("status") or "").upper() in {"COMPLETADO", "MODIFICADO"}
]
HISTORY_LOG_BY_DATE = {str(x.get("session_date")): x for x in HISTORY_LOGS if x.get("session_date")}
ALL_PLAN_SESSIONS = get_all_plan_sessions(limit=4000)
_HISTORY_SESSION_BY_ID = {int(x["id"]): x for x in ALL_PLAN_SESSIONS if x.get("id") is not None}

# V8.1.5 · Los entrenamientos adicionales viven fuera del plan base.
EXTRA_READY = extra_sessions_storage_ready()
EXTRA_SESSIONS = get_extra_sessions(ACTIVE_PLAN["id"]) if EXTRA_READY and ACTIVE_PLAN else []
EXTRA_BY_DATE = {str(x.get("session_date")): x for x in EXTRA_SESSIONS}
_VALID_TRAINING_DATES = set(PLAN_BY_DATE) | set(EXTRA_BY_DATE)

# Los registros reales del plan activo y de adicionales válidos alimentan carga/KPI.
# Un adicional nunca mejora el cumplimiento del plan: solo suma carga/volumen real.
CURRENT_LOGS = [
    x for x in LOGS
    if str(x.get("session_date")) in _VALID_TRAINING_DATES
]
ORPHAN_LOGS = get_unassigned_logs() + [
    x for x in LOGS
    if str(x.get("session_date")) not in _VALID_TRAINING_DATES
]
...
ALL_READINESS_ROWS = get_all_readiness_rows(limit=1000) if ADAPTIVE_READY else []
...
def readiness_session_guidance(readiness_row, session):
    ...
    gait = bool(rr.get("pain_changes_gait"))
    planned = float((session or {}).get("planned_km") or 0)
    is_extra = bool((session or {}).get("is_extra"))
    ...
    if status == "GREEN":
        ...
        if is_extra:
            minutes = int((session or {}).get("planned_minutes") or 0)
            time_txt = f" durante {minutes} min" if minutes else ""
            return f"Si decides hacer el entrenamiento adicional, mantenlo fácil{time_txt}{km_txt and f' (aprox.{km_txt})'} a RPE {rlo}–{rhi}/10. No lo conviertas en una sesión de calidad."
...
def adaptation_snapshot(trigger_day=None):
    ...
    if hard_flag:
        ...
        if adapted_future and (
            ...
            decision = "RESTORE"
            severity = "recovery"
            reasons.append("Los indicadores se han normalizado; puede restaurarse el plan hacia su plan original.")
        else:
            decision = "MAINTAIN"
            severity = "normal"
            reasons.append("Los datos recientes no justifican cambiar la progresión prevista.")
    else:
        reasons.append("Aún faltan datos suficientes. Completa registros diarios y entrenamientos para personalizar la adaptación.")
...
def _adapt_session_fields(session, decision, severity, quality_index=0):
    ...
    if decision == "REDUCE":
        ...
        new_km = max(2.0, round(base["planned_km"] * factor, 1))
        if major and kind in ("Series", "Tempo", "Específico"):
            return {
                "planned_km": new_km,
                "workout_type": "RODAJE",
...
def apply_adaptation(recommendation, trigger_day=None):
    ...
    try:
        for p in sorted(targets, key=lambda x: str(x.get("session_date"))):
            ...
            if workout_kind(p) in ("Series", "Tempo", "Específico"):
                quality_index += 1
...
# ============================================================
# V7.2 · Motor de reorganización del plan RCP
# ============================================================
...
def _hard_session(session):
    return workout_kind(session) in ("Series", "Tempo", "Específico", "Larga", "Carrera")


def _quality_session(session):
    return workout_kind(session) in ("Series", "Tempo", "Específico")
...
def _find_safe_replan_target(missed_session, missed_day, trigger_day, kind):
    ...
    for offset in range((end - start).days + 1):
        ...
        if globals().get("EXTRA_BY_DATE", {}).get(d.isoformat()):
            # Un adicional manual ocupa ese hueco sin formar parte del plan base.
            continue
        log = _logged_on_date(d)
        if log and str(log.get("status") or "").upper() in ("COMPLETADO", "MODIFICADO", "OMITIDO"):
            continue
        if existing and workout_kind(existing) in ("Carrera", "Series", "Tempo", "Específico", "Larga"):
            continue
...
def replan_snapshot(trigger_day=None):
    """Clasifica una sesión omitida y propone una reorganización del plan conservadora y auditable."""
    ...
    elif reason_group == "SCHEDULE" and kind in ("Series", "Tempo", "Específico"):
...
def _return_session_fields(session, factor, mode="RETURN"):
    ...
    if kind == "Larga":
        return {
            ...
            "description": "V7.2: tirada larga reducida por reorganización del plan. Mantener esfuerzo cómodo y detener si reaparecen síntomas o dolor relevante.",
        }
    return {
        "planned_km": new_km,
...
def apply_replan(recommendation, trigger_day=None):
    if not REPLAN_READY or not ACTIVE_PLAN or not recommendation:
        return False, "La reorganización del plan V7.2 no está disponible."
    ...
    if not record:
        return False, "No fue posible crear la auditoría de reorganización del plan."
    ...
    try:
        if decision in ("SKIP", "REVIEW_GOAL", "REASSESS"):
            ...
            if not scope_start or not scope_end:
                raise RuntimeError("No fue posible definir el bloque de reorganización del plan.")
            future = [
                p for p in PLAN
                if (d := parse_date_safe(p.get("session_date")))
        ...
        return True, f"Reorganización del plan V7.2 aplicada · {len(changes)} sesión(es) ajustada(s)."
    except Exception as exc:
        ...
        return False, f"No fue posible aplicar la reorganización del plan: {exc}"
...
def revert_last_replan():
    ...
    if not isinstance(changes, list):
        return False, "La auditoría de esta reorganización del plan no es válida."

    # No revierte una reorganización del plan si alguna sesión afectada ya fue ejecutada/registrada.
    for ch in changes:
        ...
        if current_date:
            ...
            if log and str(log.get("status") or "").upper() in ("COMPLETADO", "MODIFICADO", "OMITIDO"):
                return False, "No se puede revertir: una de las sesiones reorganizadas ya tiene un registro."
    ...
    try:
        ...
        return True, "Última reorganización del plan revertida."
    except Exception as exc:
        return False, f"No fue posible revertir la reorganización del plan: {exc}"


def revert_replan_record(record):
    """Revierte una reorganización del plan concreta si ninguna sesión afectada fue ya registrada."""
    if not record:
        return True, "No había reorganización del plan asociada."
    status = str(record.get("status") or "").upper()
    if status in ("REVERTED", "FAILED"):
        return True, "La reorganización del plan asociada ya estaba cerrada."
    changes = record.get("changes") or []
    if not isinstance(changes, list):
        return False, "La auditoría de la reorganización del plan asociada no es válida."
    ...
    for ch in changes:
        ...
        if current_date:
            ...
            if log and str(log.get("status") or "").upper() in ("COMPLETADO", "MODIFICADO", "OMITIDO"):
                return False, "No puedo cancelar esta ausencia porque una sesión afectada por la reorganización del plan ya tiene un registro."
    try:
        for ch in reversed(changes):
            if ch.get("op") == "UPDATE":
                update_plan_session_fields(ch["session_id"], **(ch.get("before") or {}))
        ...
        return True, "Reorganización del plan asociada revertida."
    except Exception as exc:
        return False, f"No fue posible revertir la reorganización del plan asociada: {exc}"


def cancel_planned_absence(log_row, session_row):
    """Cancela una ausencia futura y deshace su reorganización del plan si ya fue aplicada."""
...
def weekly_review_snapshot(day_value):
    ...
    # Los adicionales no mejoran adherencia ni km del plan, pero sus señales de
    # recuperación sí deben contar para seguridad semanal.
    for _dstr, _elog in local_logs.items():
        if _dstr not in globals().get("EXTRA_BY_DATE", {}):
            continue
        if str(_elog.get("status") or "").upper() not in ("COMPLETADO", "MODIFICADO"):
            continue
        if _elog.get("post_pain") is not None:
            post_pains.append(int(_elog.get("post_pain")))
        if _elog.get("post_fatigue") is not None:
            post_fatigues.append(int(_elog.get("post_fatigue")))
...
def all_weekly_stats():
    ...
    for log in CURRENT_LOGS:
        if str(log.get("status") or "").upper() not in ("COMPLETADO", "MODIFICADO"):
            continue
        _log_date = parse_date_safe(log.get("session_date"))
        p = PLAN_BY_DATE.get(str(log.get("session_date")))
        if p:
            week = int(p.get("week_no") or 0)
        elif _log_date and str(log.get("session_date")) in EXTRA_BY_DATE:
            week = _week_no_for_date(_log_date, 0)
        else:
            continue
...
def longitudinal_calendar_weekly_stats():
    ...
    def ensure(day):
        d = parse_date_safe(day) if not isinstance(day, date) else day
        if not d:
            return None
    ...
    # Plan ACTIVO: solo para comparación y cumplimiento.
    for sess in PLAN:
        d = parse_date_safe(sess.get("session_date"))
        if not d or session_is_optional(sess):
            continue
        item = ensure(d)
        if not item:
            continue
        item["plan"] += float(sess.get("planned_km") or 0)
        if d <= rcp_today():
            item["due"] += 1
            status = str(LOG_BY_DATE.get(d.isoformat(), {}).get("status") or "").upper()
            if status in {"COMPLETADO", "MODIFICADO"}:
    ...
    # Historia REAL: todos los planes activos/archivados, deduplicada por fecha.
    for log in HISTORY_COMPLETED_LOGS:
        d = parse_date_safe(log.get("session_date"))
        if not d:
            continue
        item = ensure(d)
        ...
        if log.get("rpe") is not None:
            try:
                item["rpes"].append(float(log.get("rpe")))
            except Exception:
                pass
        try:
            sec = float(log.get("actual_duration_sec") or 0)
            rpe = float(log.get("rpe") or 0)
            if sec > 0 and 1 <= rpe <= 10:
                item["load"] += (sec / 60.0) * rpe
        except Exception:
            pass
...
def render_goal_hero():
    ...
    display_name_safe = html.escape(str(profile.get("display_name") or "Corredor"))
    st.markdown(
        f"""
        <div class="rcp-hero">
...
def session_display_name(session):
    """Nombre visible en lenguaje claro, sincronizado con los km vigentes."""
    ...
    if re.search(r"\s+\d+(?:[\.,]\d+)?\s*km\s*$", name, flags=re.IGNORECASE):
        ...
        name = f"{base} {km:g} km" if km > 0 else base
    name = re.sub(r"^Tempo\b", "Ritmo sostenido (tempo)", name, flags=re.IGNORECASE)
    name = re.sub(r"^Run[–-]Walk\b", "Correr / caminar", name, flags=re.IGNORECASE)
    name = re.sub(r"^Fartlek\b", "Cambios de ritmo (fartlek)", name, flags=re.IGNORECASE)
    return name
...
def build_plan_pdf_bytes(
    ...
    runner_name = _pdf_clean((profile_row or {}).get("display_name") or "Corredor")
    race_date = parse_date_safe((active_goal or {}).get("race_date"))
    ...
    cards2 = Table([[
        ...
        metric_card("Volumen total", volume_text, teal),
    ]], colWidths=[56.5*mm, 56.5*mm, 56.5*mm])
    ...
    if _pdf_zone_rows:
        ...
        story.append(ztable)
        story.append(P(f"Fuente: {_pdf_v8.get('source')} · confianza {_pdf_v8.get('confidence')}. El esfuerzo percibido (RPE) y la prueba del habla tienen prioridad sobre la cifra.", "RCPSmall"))
    ...
    for week_no, week_sessions in week_groups.items():
        ...
        race_km = sum(float(p.get("planned_km") or 0) for p in week_sessions if not session_is_optional(p) and workout_kind(p) == "Carrera")
        training_km = max(0.0, km - race_km)
        long_km = max([float(p.get("planned_km") or 0) for p in week_sessions if workout_kind(p) == "Larga"] or [0.0])
        phase = _pdf_clean(ui_phase_label((week_meta.get(week_no) or {}).get("phase")))
        quality = sum(1 for p in week_sessions if workout_kind(p) in ("Series", "Tempo", "Específico", "Carrera"))
        weekly.append({
            "week": week_no,
    ...
    if weekly:
        ...
        bar.height = 35 * mm
        bar.width = 88 * mm
        bar.data = [[x["training_km"] for x in weekly]]
        bar.categoryAxis.categoryNames = [f"S{x['week']}" for x in weekly]
        bar.valueAxis.valueMin = 0
        max_km = max([x["training_km"] for x in weekly] or [1])
        ...
        chart.add(bar)
        chart.add(String(10*mm, 48*mm, "Kilómetros de entrenamiento", fontName="Helvetica-Bold", fontSize=8, fillColor=ink))
    ...
    weekly_rows = [[
        ...
        P("Entreno", "RCPSmall"), P("Carrera", "RCPSmall"), P("Larga", "RCPSmall"), P("Calidad/comp.", "RCPSmall"),
    ]]
    for w in weekly:
        weekly_rows.append([
            P(str(w["week"])),
            ...
            P(w["phase"]),
            P(f"{w['training_km']:g}"),
            P(f"{w['race_km']:g}" if w["race_km"] else "—"),
            P(f"{w['long']:g}" if w["long"] else "—"),
            P(str(w["quality"])),
        ])
    wt = Table(weekly_rows, colWidths=[16*mm, 29*mm, 32*mm, 21*mm, 21*mm, 20*mm, 27*mm], repeatRows=1)
    ...
    if mode == "Resumido":
        ...
        story.append(Paragraph("Plan día a día", styles["RCPH1"]))
        story.append(P("Cada tarjeta refleja el plan vigente al generar el PDF. Los ajustes o reorganizaciones posteriores aparecerán cuando generes un PDF nuevo.", "RCPSubtitle"))

        day_short = ["LUN", "MAR", "MIÉ", "JUE", "VIE", "SÁB", "DOM"]
        for week_no, week_sessions in week_groups.items():
            story.append(CondPageBreak(82 * mm))
            wm = week_meta.get(week_no) or {}
            phase = _pdf_clean(ui_phase_label(wm.get("phase")))
            pcolor = phase_color(phase)
            wk_km = sum(float(p.get("planned_km") or 0) for p in week_sessions if not session_is_optional(p))
            wk_race_km = sum(float(p.get("planned_km") or 0) for p in week_sessions if not session_is_optional(p) and workout_kind(p) == "Carrera")
            wk_training_km = max(0.0, wk_km - wk_race_km)
            wk_long = max([float(p.get("planned_km") or 0) for p in week_sessions if workout_kind(p) == "Larga"] or [0.0])
            wk_quality = sum(1 for p in week_sessions if workout_kind(p) in ("Series", "Tempo", "Específico", "Carrera"))
            if wk_race_km > 0:
                _wk_volume_html = f'<font color="#FFFFFF"><b>Entreno {wk_training_km:.1f} km</b></font><br/><font size="7" color="#E2E8F0">Carrera {wk_race_km:g} km · Total {wk_km:.1f} km · Larga {wk_long:g} · Calidad/comp. {wk_quality}</font>'
            else:
                _wk_volume_html = f'<font color="#FFFFFF"><b>{wk_training_km:.1f} km entrenamiento</b></font><br/><font size="7" color="#E2E8F0">Larga {wk_long:g} km · Calidad/comp. {wk_quality}</font>'
            ...
            week_header = Table([[
                ...
                Paragraph(_wk_volume_html, ParagraphStyle("WeekStats", parent=styles["RCPBody"], alignment=TA_LEFT, leading=11)),
            ...
            for p in week_sessions:
                ...
                if REPLAN_READY and str(p.get("replan_status") or "BASELINE").upper() != "BASELINE":
                    tags.append("Reorganizada por RCP")
...
if current_page == "Hoy":
    ...
    today_extra = EXTRA_BY_DATE.get(selected_day.isoformat()) if EXTRA_READY else None
    today_context_session = today_session or (extra_session_to_session(today_extra) if today_extra else None)
    today_log = LOG_BY_DATE.get(selected_day.isoformat())

    # V7.1.1 · Estado para entrenar compacto. El formulario permanece cerrado por defecto.
    coach = None
    if ADAPTIVE_READY and selected_day == rcp_today():
        ...
        if existing_ready:
            st.markdown(
                readiness_summary_html(existing_ready, today_context_session),
                unsafe_allow_html=True,
            )
            checkin_label = "✏️ Actualizar registro diario"
        else:
            st.markdown(
                '<div class="rcp-readiness"><div class="rcp-readiness-title"><strong>⚪ Sin registro diario de hoy</strong>'
                '<span class="rcp-readiness-score">—/100</span></div>'
                '<div>Completa un registro diario breve para contextualizar la sesión y alimentar el motor adaptativo.</div>'
                '<div class="rcp-session-guidance"><b>Impacto en hoy:</b> hasta completar el registro diario, RunningCoachPro mantiene la prescripción sin inferir tu estado de recuperación.</div></div>',
                unsafe_allow_html=True,
            )
            checkin_label = "📝 Hacer registro diario"
        ...
        # Se calcula aquí, pero el Entrenador se presenta DESPUÉS de la sesión del día.
        coach = adaptation_snapshot(rcp_today())

    elif not ADAPTIVE_READY:
        st.warning("Motor adaptativo V7.1 pendiente: ejecuta supabase_v7_1_adaptive.sql para activar registro diario y ajustes dinámicos.")

    with st.container(border=True):
        if not today_session:
            if today_extra:
                _extra_view = extra_session_to_session(today_extra)
                _extra_log = LOG_BY_DATE.get(selected_day.isoformat()) or {}
                st.markdown("### 🏃 Entrenamiento adicional")
                st.caption("Opcional · no modifica, mueve ni reemplaza ninguna sesión del plan base.")
                st.markdown(f"## {str(today_extra.get('workout_type') or 'Rodaje suave')}")
                _ex1, _ex2, _ex3 = st.columns(3)
                _ex1.metric("Tiempo", f"{int(today_extra.get('planned_minutes') or 0)} min")
                _ex2.metric("Distancia", "Por tiempo" if not float(today_extra.get('planned_km') or 0) else f"{float(today_extra.get('planned_km') or 0):g} km")
                _ex3.metric("Esfuerzo", f"RPE {int(today_extra.get('target_rpe') or 3)}/10")
                st.caption(rpe_reference(int(today_extra.get("target_rpe") or 3)))
                if today_extra.get("notes"):
                    st.write(str(today_extra.get("notes")))
                st.info("Este entrenamiento suma a tu carga y kilómetros reales, pero no cuenta como cumplimiento de una sesión que hayas omitido otro día.")
                e1, e2 = st.columns(2)
                if e1.button("✅ Registrar entrenamiento adicional", type="primary", use_container_width=True, key=f"extra_register_home_{today_extra.get('id')}"):
                    set_page("Registro", selected_day)
                    st.rerun()
                if e2.button("🗑️ Quitar adicional", use_container_width=True, key=f"extra_delete_home_{today_extra.get('id')}"):
                    delete_extra_session(today_extra.get("id"), selected_day.isoformat(), delete_training_log=True)
                    st.session_state["rcp_saved_notice"] = "Entrenamiento adicional eliminado. El plan base no cambió."
                    st.rerun()
            else:
                ...
                if EXTRA_READY:
                    with st.expander("➕ Añadir entrenamiento adicional", expanded=False):
                        st.caption(
                            "Úsalo cuando excepcionalmente tengas tiempo en un día libre. "
                            "No mueve ni borra el plan; si omitiste otra sesión, esa omisión seguirá registrada."
                        )
                        _extra_ready_row = READINESS_BY_DATE.get(selected_day.isoformat()) or {}
                        _extra_blocked = (
                            selected_day == rcp_today()
                            and (
                                bool(_extra_ready_row.get("illness"))
                                or bool(_extra_ready_row.get("pain_changes_gait"))
                                or int(_extra_ready_row.get("pain") or 0) >= 7
                                or str(_extra_ready_row.get("readiness_status") or "").upper() == "RED"
                            )
                        )
                        if _extra_blocked:
                            st.error("Con el estado registrado hoy RCP no recomienda añadir trote. Mantén la recuperación y vuelve a valorar tu estado después.")
                        _extra_cap = training_time_limit_minutes(
                            selected_day,
                            (LATEST_ASSESSMENT or {}).get("answers") or {},
                            profile,
                        )
                        # V8.1.6 · Los entrenamientos adicionales son libres: la disponibilidad habitual
                        # y las sesiones cercanas pueden contextualizar la decisión, pero no imponen un
                        # techo automático al tiempo que el usuario decide registrar.
                        if _extra_cap:
                            st.caption(
                                f"Disponibilidad habitual guardada para {DAY_NAMES[selected_day.weekday()]}: {_extra_cap} min. "
                                "Es solo una referencia; este entrenamiento adicional no tiene un límite automático."
                            )
                        else:
                            st.caption("Elige libremente cuánto tiempo quieres dedicar a este entrenamiento adicional.")
                        with st.form(f"extra_create_home_{selected_day.isoformat()}"):
                            _et1, _et2 = st.columns(2)
                            _extra_type = _et1.selectbox(
                                "Tipo",
                                ["Rodaje suave", "Recuperación", "Caminata / trote"],
                                index=0,
                            )
                            _extra_minutes = _et2.number_input(
                                "Tiempo objetivo (min)",
                                min_value=10,
                                value=40,
                                step=5,
                                help="Tú eliges la duración. RCP no la reduce automáticamente por otra sesión del plan.",
                            )
                            _ek1, _ek2 = st.columns(2)
                            _extra_km = _ek1.number_input(
                                "Distancia objetivo (km, opcional)",
                                min_value=0.0,
                                value=0.0,
                                step=0.5,
                                help="Déjalo en 0 si prefieres correr solo por tiempo. No hay un máximo fijo para el entrenamiento adicional.",
                            )
                            _extra_rpe = _ek2.selectbox(
                                "RPE objetivo",
                                [2, 3, 4],
                                index=1,
                                format_func=rpe_option_label,
                            )
                            st.caption(rpe_reference(_extra_rpe))
                            _extra_notes = st.text_area("Nota opcional", placeholder="Ej.: trote muy suave porque hoy tuve tiempo.")
                            _extra_submit = st.form_submit_button(
                                "💾 Guardar entrenamiento adicional",
                                use_container_width=True,
            ...
            st.caption(
                ...
                + (f" · Fase {ui_phase_label(_phase)}" if _phase else "")
            )
    ...
    # V7.1.1 · Entrenador después de la sesión: Home prioriza readiness → entrenamiento → adaptación.
    if ADAPTIVE_READY and selected_day == rcp_today() and coach is not None:
        ...
        with st.expander("Ver indicadores del Entrenador", expanded=False):
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Adherencia 14 d", "—" if metrics.get("adherence_pct") is None else f"{metrics['adherence_pct']:.0f}%")
            m2.metric("Estado para entrenar · 7 días", "—" if metrics.get("readiness_avg_7d") is None else f"{metrics['readiness_avg_7d']:.0f}/100")
            m3.metric("Esfuerzo por encima de lo previsto", f"+{float(metrics.get('avg_rpe_excess') or 0):.1f}")
        ...
        if decision in ("PROTECT", "REDUCE", "RESTORE"):
            if already_today:
                st.caption("Esta recomendación ya fue aplicada hoy. Los cambios quedan guardados en el historial de ajustes.")
            else:
    ...
    if REPLAN_READY and selected_day == rcp_today():
        ...
        if replan:
            ...
            st.markdown("### 🔄 Reorganización del plan")
            with st.container(border=True):
                ...
                if decision == "REASSESS":
                    ...
                    if st.button("🔄 Aplicar propuesta", use_container_width=True, type="primary"):
                        ok, msg = apply_replan(replan, rcp_today())
                        (st.success if ok else st.error)(msg)
                        if ok:
                            st.rerun()
    elif ADAPTIVE_READY and not REPLAN_READY and selected_day == rcp_today():
        st.caption("🔄 Reorganización del plan V7.2 pendiente de activar en Supabase.")
...
# ============================================================
    ...
    if _week_sessions:
        ...
        if _wi:
            title_col.caption(
                f"Fase {ui_phase_label(_wi.get('phase'))} · Objetivo {_wi.get('target_km') or '—'} km · "
        ...
        if not WEEKLY_REVIEW_READY:
            ...
            if _weekly_live:
                ...
                with st.expander(
                    ...
                    r3.metric("Estado para entrenar", "—" if _wm.get("readiness_avg") is None else f"{float(_wm['readiness_avg']):.0f}/100")
    ...
    for i in range(7):
        ...
        with st.container(border=True):
            ...
            c1.caption(d.strftime("%d/%m"))
            if s:
                c2.markdown(f"**{session_display_name(s)}**")
                ...
                _extra_week = EXTRA_BY_DATE.get(d.isoformat()) if EXTRA_READY else None
                if _extra_week:
                    _extra_week_log = LOG_BY_DATE.get(d.isoformat()) or {}
                    c2.markdown(f"**🏃 {str(_extra_week.get('workout_type') or 'Entrenamiento adicional')}**")
                    c2.caption(
                        f"Adicional opcional · {int(_extra_week.get('planned_minutes') or 0)} min · "
                        f"RPE {int(_extra_week.get('target_rpe') or 3)}/10"
                    )
                    if float(_extra_week.get("planned_km") or 0):
                        c3.markdown(f"**{float(_extra_week.get('planned_km') or 0):g} km**")
                    else:
                        c3.markdown("**Por tiempo**")
                    c3.caption(extra_session_status_label(_extra_week, _extra_week_log))
                    if c4.button("Abrir", key=f"week_open_extra_{_extra_week['id']}", use_container_width=True):
                        set_page("Registro", d)
                        st.rerun()
                else:
...
# ============================================================
    ...
    with st.container(border=True):
        ...
        st.caption("Tus entrenamientos realizados se conservan aunque RCP archive un plan y cree una versión nueva. Este bloque reúne actividades de planes activos y archivados sin contarlas dos veces.")
        _hist_completed = list(HISTORY_COMPLETED_LOGS)
        _hist_km = sum(float(x.get("actual_km") or 0) for x in _hist_completed)
        _hist_sec = sum(float(x.get("actual_duration_sec") or 0) for x in _hist_completed)
        _hist_rpes = [float(x.get("rpe")) for x in _hist_completed if x.get("rpe") is not None]
        _h1, _h2, _h3, _h4 = st.columns(4)
        _h1.metric("Sesiones registradas", len(_hist_completed))
        _h2.metric("KM acumulados", f"{_hist_km:.1f}")
        _h3.metric("Tiempo acumulado", f"{_hist_sec/3600:.1f} h" if _hist_sec > 0 else "—")
        _h4.metric("RPE promedio", "—" if not _hist_rpes else f"{sum(_hist_rpes)/len(_hist_rpes):.1f}/10")

        if _hist_completed:
            _history_rows = []
            for _log in sorted(_hist_completed, key=lambda x: str(x.get("session_date") or ""), reverse=True):
                _sess = _HISTORY_SESSION_BY_ID.get(int(_log.get("plan_session_id") or 0))
                _km = float(_log.get("actual_km") or 0)
                _sec = float(_log.get("actual_duration_sec") or 0)
                _pace = "—"
                if _km > 0 and _sec > 0:
                    _pace_total = _sec / _km
                    _pace = f"{int(_pace_total//60)}:{int(round(_pace_total%60)):02d}/km"
                _history_rows.append({
                    "Fecha": str(_log.get("session_date") or ""),
                    "Entrenamiento": session_display_name(_sess) if _sess else "Entrenamiento registrado",
                    "KM": round(_km, 2),
                    "Tiempo": fmt_time(int(_sec)) if _sec > 0 else "—",
                    "Ritmo": _pace,
                    "RPE": _log.get("rpe") if _log.get("rpe") is not None else "—",
                    "FC": _log.get("avg_hr") or "—",
                    "Superficie": _log.get("training_surface_used") or "—",
                })
            with st.expander("Ver todos mis entrenamientos guardados", expanded=True):
                st.dataframe(_history_rows, use_container_width=True, hide_index=True)
        else:
            st.info("Todavía no hay entrenamientos completados guardados en el historial.")
    ...
    with st.container(border=True):
        st.markdown("### 🧠 Estado actual del corredor")
        st.markdown(f"**{_v8_state['headline']}** · {_v8_state['focus']}")
        v1, v2, v3, v4 = st.columns(4)
        v1.metric("Recuperación", _v8_rec.get("status") or "—", None if _v8_rec.get("readiness_avg") is None else f"Estado para entrenar {_v8_rec['readiness_avg']:.0f}/100")
        v2.metric("Carga esta semana", f"{float(_v8_load.get('current_week_load') or 0):.0f}", "minutos × RPE")
        v3.metric("Tendencia aeróbica", _v8_trend.get("status") or "—", None if _v8_trend.get("delta_pct") is None else f"{_v8_trend['delta_pct']:+.1f}% a esfuerzo 3.5/10")
        v4.metric("Zonas", str(_v8_zones.get("confidence") or "provisional").title(), f"{int(_v8_zones.get('calibration_count') or 0)} sesiones usadas para aprender")
        st.caption(_v8_rec.get("message") or "")
        _guard = (_v8_state.get("guardrails") or {})
        if _guard.get("status") == "REVISAR":
            st.warning("Límite de seguridad del plan: " + " ".join(_guard.get("issues") or []))
        else:
            st.caption("✅ Límites de seguridad: la distribución de intensidad y la progresión futura no muestran alertas estructurales.")

    st.markdown("### 🎯 Ritmos y zonas de entrenamiento")
    st.caption(
        "Se actualizan con una prueba o marca reciente y, en las zonas aeróbicas, con tu relación real entre ritmo y esfuerzo. "
    ...
    if _zone_rows:
        ...
        st.info("Todavía no hay una referencia de rendimiento suficiente para calcular ritmos y zonas; RCP mantiene prescripción por RPE (esfuerzo percibido) y prueba del habla.")

    weekly_all = longitudinal_calendar_weekly_stats()
    period = st.selectbox(
        "Periodo",
        ["Últimas 4 semanas", "Últimas 8 semanas", "Hasta hoy", "Plan completo"],
        index=0,
        key="progress_period_v826",
    )

    current_monday, _ = week_bounds(rcp_today())
    if period == "Últimas 4 semanas":
        cutoff = current_monday - timedelta(weeks=3)
        weekly_view = [w for w in weekly_all if cutoff <= w["start"] <= current_monday]
    elif period == "Últimas 8 semanas":
        cutoff = current_monday - timedelta(weeks=7)
        weekly_view = [w for w in weekly_all if cutoff <= w["start"] <= current_monday]
    elif period == "Hasta hoy":
        weekly_view = [w for w in weekly_all if w["start"] <= current_monday]
    else:
        weekly_view = weekly_all

    # Rango temporal real de la vista. Se usa en todos los gráficos longitudinales.
    if weekly_view:
        view_start = min(w["start"] for w in weekly_view)
        view_end = max(w["end"] for w in weekly_view)
    else:
        view_start = current_monday
        view_end = current_monday + timedelta(days=6)
    ...
    p1, p2, p3, p4 = st.columns(4)
    p1.metric("KM plan activo", f"{plan_km_view:.1f}")
    p2.metric("KM real", f"{real_km_view:.1f}")
    p3.metric("Cumplimiento plan activo", "—" if compliance_view is None else f"{compliance_view:.0f}%")
    p4.metric("Carga por esfuerzo", "—" if load_view <= 0 else f"{load_view:.0f}")
    st.caption("Los KM reales incluyen entrenamientos guardados en planes anteriores. El cumplimiento solo corresponde al plan actualmente activo.")

    # 1 · Plan vs real
    st.markdown("### Plan vs real por semana calendario")
    values = []
    for w in weekly_view:
        values.extend([
            {"Semana": w["label"], "Serie": "Plan activo", "KM": round(w["plan"], 1)},
            {"Semana": w["label"], "Serie": "Real", "KM": round(w["real"], 1)},
        ])
    if values:
        st.vega_lite_chart(
    ...
    with c_right:
        st.markdown("### Esfuerzo percibido semanal (RPE)")
        rpe_values = [
            {
                "Semana": w["label"],
    ...
    # 3 · Carga por esfuerzo
    st.markdown("### Carga semanal según duración y esfuerzo")
    load_values = [
        {"Semana": w["label"], "Carga": round(w["load"], 0)}
    ...
    if load_values:
        ...
        st.caption("Carga por esfuerzo = duración real de la sesión en minutos × RPE. Solo usa registros con duración y RPE.")
    else:
    ...
    long_values = []
    # Plan activo dentro del rango seleccionado.
    for p in PLAN:
        d = parse_date_safe(p.get("session_date"))
        if not d or not (view_start <= d <= view_end) or workout_kind(p) != "Larga":
            continue
        long_values.append({
            "Fecha": d.isoformat(),
            "Serie": "Plan activo",
    ...
    # Tiradas largas realmente realizadas, aunque pertenezcan a un plan archivado.
    for log in HISTORY_COMPLETED_LOGS:
        d = parse_date_safe(log.get("session_date"))
        if not d or not (view_start <= d <= view_end):
            continue
        sess = _HISTORY_SESSION_BY_ID.get(int(log.get("plan_session_id") or 0)) or PLAN_BY_DATE.get(str(log.get("session_date"))) or {}
        if workout_kind(sess) != "Larga":
            continue
        long_values.append({
            "Fecha": d.isoformat(),
            "Serie": "Real",
    ...
    # 5 · Ritmo + FC · historia transversal a todas las versiones del plan
    pace_values = []
    hr_values = []
    type_km = {}
    for log in HISTORY_COMPLETED_LOGS:
        ...
        if not d or not (view_start <= d <= view_end):
            continue
        p = _HISTORY_SESSION_BY_ID.get(int(log.get("plan_session_id") or 0)) or PLAN_BY_DATE.get(str(log.get("session_date"))) or {}
        km = float(log.get("actual_km") or 0)
        sec = float(log.get("actual_duration_sec") or 0)
        session_name = session_display_name(p) if p else "Entrenamiento registrado"
        if km > 0 and sec > 0:
            pace_values.append({
                "Fecha": d.isoformat(),
        ...
        kind = workout_kind(p) if p else "Otro"
    ...
    # 7 · V7.1 Estado para entrenar + historial de adaptaciones
    if ADAPTIVE_READY:
        st.divider()
        st.markdown("## 🧠 Ajustes del plan")
        readiness_values = []
        for r in sorted(ALL_READINESS_ROWS, key=lambda x: str(x.get("checkin_date"))):
            readiness_values.append({
                "Fecha": str(r.get("checkin_date")),
                "Estado para entrenar": int(r.get("readiness_score") or 0),
                "Estado": str(r.get("readiness_status") or ""),
            })
        if readiness_values:
            st.markdown("### Estado diario para entrenar")
            st.vega_lite_chart(
                {
                    ...
                    "encoding": {
                        "x": {"field": "Fecha", "type": "temporal", "title": None},
                        "y": {"field": "Estado para entrenar", "type": "quantitative", "scale": {"domain": [0, 100]}, "title": "0–100"},
                        "color": {"field": "Estado", "type": "nominal"},
                        "tooltip": [{"field": "Fecha", "type": "temporal"}, {"field": "Estado para entrenar"}, {"field": "Estado"}],
                    },
                    "height": 250,
                },
                use_container_width=True,
        ...
        if ADJUSTMENTS:
            ...
            for a in ADJUSTMENTS[:15]:
                ...
                adj_rows.append({
                    ...
                    "Estado": ui_status_label(a.get("status")),
                })
    ...
    # 8 · V7.2 Reorganización del plan
    if REPLAN_READY:
        st.divider()
        st.markdown("## 🔄 Reorganización del plan")
        if REPLANS:
            ...
            for r in REPLANS[:20]:
                ...
                rows.append({
                    ...
                    "Estado": ui_status_label(r.get("status")),
                })
            st.dataframe(rows, use_container_width=True, hide_index=True)
            latest = next((r for r in REPLANS if str(r.get("status") or "").upper() == "APPLIED"), None)
            if latest:
                if st.button("↩️ Revertir última reorganización del plan", use_container_width=True):
                    ok, msg = revert_last_replan()
                    (st.success if ok else st.error)(msg)
                    if ok:
                        st.rerun()
        else:
            st.info("Todavía no hay reorganizaciones del plan. Se crean cuando una sesión se registra como OMITIDA —incluidas ausencias futuras conocidas— y RCP propone una acción.")
    ...
    l1.metric("Carga semanal por esfuerzo", f"{float(_load.get('current_week_load') or 0):.0f}")
    ...
    l4.metric("Repetitividad de carga", "—" if _load.get("monotony") is None else f"{float(_load['monotony']):.2f}")
    st.caption("Carga por esfuerzo = minutos de ejercicio × esfuerzo percibido (RPE). La repetitividad y la exigencia semanal se muestran solo como descriptores; RCP no las usa para predecir lesiones.")
    ...
    st.markdown("### 🧪 Pruebas RCP")
    _test_rec = v8_test_recommendation()
    if not CORE_V8_READY:
        st.warning("Ejecuta `supabase_v8_0_core_complete.sql` para activar el historial de pruebas RCP.")
    else:
        if _test_rec.get("recommended"):
            sd = _test_rec.get("suggested_date")
            st.info(
                f"RCP recomienda una **prueba {_test_rec.get('test_type')}** alrededor del "
                f"**{sd.strftime('%d/%m/%Y') if sd else 'próximo bloque'}**. {_test_rec.get('reason')}"
            )
        else:
            st.success(f"No necesitas una prueba ahora. {_test_rec.get('reason')}")

        with st.expander("➕ Registrar una prueba realizada", expanded=False):
            with st.form("v8_test_form"):
                tc1, tc2 = st.columns(2)
                test_type = tc1.selectbox("Tipo de prueba", ["3K", "5K", "10K", "20MIN"], index=1)
            ...
            if save_test:
                if duration_sec <= 0 or distance_km <= 0:
                    ...
                    st.success("Prueba guardada. Tus ritmos y zonas se recalcularán con esta referencia de rendimiento.")
                    st.rerun()

        if PERFORMANCE_TESTS:
            with st.expander("Historial de pruebas", expanded=False):
                st.dataframe([
                    {
                        "Fecha": t.get("test_date"),
                        "Prueba": t.get("test_type"),
                        "Distancia": f"{float(t.get('distance_km') or 0):g} km",
                        "Tiempo": fmt_time(t.get("duration_sec")),
                        "RPE": t.get("rpe") or "—",
                        "Estado": ui_status_label(t.get("status")),
                    }
...
# ============================================================
    ...
    with st.expander("📄 Exportar mi plan a PDF", expanded=False):
        st.caption(
            "Genera una instantánea del plan activo real. Si después aplicas una adaptación o reorganización del plan, "
    ...
    for p in PLAN:
        ...
        table.append({
            ...
            "Ajuste del plan": ui_status_label(p.get("adaptation_status") or "BASELINE") if ADAPTIVE_READY else "—",
            "Reorganización del plan": ui_status_label(p.get("replan_status") or "BASELINE") if REPLAN_READY else "—",
    ...
    if filtered_plan:
        labels = {
            f"{parse_date_safe(p['session_date']).strftime('%d/%m')} · {session_display_name(p)}": p
            for p in filtered_plan
        }
        chosen = st.selectbox("Abrir detalle de una sesión", list(labels.keys()))
        p = labels[chosen]
        with st.container(border=True):
            st.markdown(f"### {session_display_name(p)}")
            st.caption(
...
# ============================================================
    ...
    extra_session = EXTRA_BY_DATE.get(selected_day.isoformat()) if EXTRA_READY else None
    if not session and not extra_session:
        st.info("Esta fecha no tiene entrenamiento programado ni adicional.")
        if EXTRA_READY:
            st.caption("Puedes añadir un trote opcional desde la pantalla Hoy seleccionando esta misma fecha.")
        nearby = [
            p for p in PLAN
            if (d := parse_date_safe(p.get("session_date"))) and d >= rcp_today()
        ][:5]
        if nearby:
            ...
            for p in nearby:
                d = parse_date_safe(p["session_date"])
                if st.button(
                    f"{d.strftime('%d/%m')} · {session_display_name(p)} · {float(p.get('planned_km') or 0):g} km",
                    key=f"reg_pick_{p['id']}",
                    use_container_width=True,
                ):
                    set_page("Registro", d)
                    st.rerun()
    elif extra_session and not session:
        existing = LOG_BY_DATE.get(selected_day.isoformat(), {})
        with st.container(border=True):
            st.caption("ENTRENAMIENTO ADICIONAL · FUERA DEL PLAN BASE")
            st.markdown(f"## 🏃 {str(extra_session.get('workout_type') or 'Rodaje suave')}")
            x1, x2, x3 = st.columns(3)
            x1.metric("Tiempo objetivo", f"{int(extra_session.get('planned_minutes') or 0)} min")
            x2.metric(
                "Distancia objetivo",
                "Por tiempo" if not float(extra_session.get("planned_km") or 0) else f"{float(extra_session.get('planned_km') or 0):g} km",
            )
            x3.metric("RPE objetivo", f"{int(extra_session.get('target_rpe') or 3)}/10")
            st.caption(rpe_reference(int(extra_session.get("target_rpe") or 3)))
            if extra_session.get("notes"):
                st.write(str(extra_session.get("notes")))
            st.info(
                "Este entrenamiento es opcional. No reemplaza ni completa automáticamente una sesión omitida de otro día. "
                "Sí cuenta como carga, kilómetros y respuesta real del corredor."
            )
            render_rpe_guide(expanded=False, key_suffix=f"extra_register_{selected_day.isoformat()}")

        with st.form(f"extra_log_form_{selected_day.isoformat()}"):
            st.markdown("### ¿Cómo salió?")
            _planned_extra_km = float(extra_session.get("planned_km") or 0)
            _er1, _er2, _er3 = st.columns(3)
            _extra_actual_km = _er1.number_input(
                "KM reales",
                0.0, 100.0,
                float(existing.get("actual_km") if existing.get("actual_km") is not None else _planned_extra_km),
                .1,
            )
            _extra_duration_default = (
                fmt_time(existing.get("actual_duration_sec"))
                if existing.get("actual_duration_sec")
                else fmt_time(int(extra_session.get("planned_minutes") or 0) * 60)
            )
            _extra_duration_text = _er2.text_input("Duración HH:MM:SS", value=_extra_duration_default)
            _extra_actual_rpe = _er3.selectbox(
                "Esfuerzo percibido (RPE)",
                list(range(1, 11)),
                index=max(0, min(9, int(existing.get("rpe") or extra_session.get("target_rpe") or 3) - 1)),
                format_func=rpe_option_label,
            )
            st.caption(rpe_reference(_extra_actual_rpe))

            _ehr1, _ehr2 = st.columns(2)
            _extra_avg_hr = _ehr1.number_input("FC media (opcional)", 0, 230, int(existing.get("avg_hr") or 0))
            _extra_max_hr = _ehr2.number_input("FC máxima (opcional)", 0, 240, int(existing.get("max_hr") or 0))
            ...
            _extra_surface_ui = st.selectbox(
            ...
            if ADAPTIVE_READY:
                ...
                _extra_post_pain = _ep1.slider("Dolor después de entrenar", 0, 10, int(existing.get("post_pain") or 0))
                _extra_post_fatigue = _ep2.slider("Fatiga después de entrenar", 1, 5, int(existing.get("post_fatigue") or 2))
                _difficulty_options = ["Mucho más fácil", "Más fácil", "Como esperaba", "Más difícil", "Mucho más difícil"]
                _current_diff = str(existing.get("perceived_difficulty") or "Como esperaba")
                _extra_difficulty = st.selectbox(
                    "La sesión se sintió…",
                    _difficulty_options,
                    index=_difficulty_options.index(_current_diff) if _current_diff in _difficulty_options else 2,
                )
            else:
                _extra_post_pain = None
                _extra_post_fatigue = None
                _extra_difficulty = None

            _extra_notes_actual = st.text_area("Observaciones", value=str(existing.get("notes") or ""))
            _extra_log_submit = st.form_submit_button("💾 Guardar entrenamiento adicional", use_container_width=True)

        if _extra_log_submit:
            _extra_seconds = parse_hms(_extra_duration_text) if _extra_duration_text else 0
            if _extra_duration_text and _extra_seconds is None:
                st.error("Duración inválida. Usa HH:MM:SS.")
            else:
                save_log({
                    "session_date": selected_day.isoformat(),
                    "plan_session_id": None,
                    "actual_km": float(_extra_actual_km),
                    "actual_duration_sec": int(_extra_seconds or 0),
                    "rpe": int(_extra_actual_rpe),
                    "avg_hr": int(_extra_avg_hr) if _extra_avg_hr else None,
                    "max_hr": int(_extra_max_hr) if _extra_max_hr else None,
                    "status": "COMPLETADO",
                    "training_surface_used": (
                        {"Exterior": "EXTERIOR", "Caminadora": "CAMINADORA"}.get(_extra_surface_ui)
                        if pace_rpe_calibration_storage_ready()
                        else None
                    ),
                    "post_pain": int(_extra_post_pain) if ADAPTIVE_READY else None,
                    "post_fatigue": int(_extra_post_fatigue) if ADAPTIVE_READY else None,
                    "perceived_difficulty": _extra_difficulty if ADAPTIVE_READY else None,
                    "notes": _extra_notes_actual.strip(),
                })
                st.session_state["rcp_saved_notice"] = (
                    "Entrenamiento adicional guardado ✅. Se suma a tu carga real sin modificar el plan base."
                )
                set_page("Hoy", selected_day)
                st.rerun()

        st.divider()
        _del_extra_label = "🗑️ Eliminar adicional y su registro" if existing else "🗑️ Quitar entrenamiento adicional"
        if st.button(_del_extra_label, key=f"delete_extra_register_{extra_session.get('id')}", use_container_width=True):
            delete_extra_session(extra_session.get("id"), selected_day.isoformat(), delete_training_log=True)
            st.session_state["rcp_saved_notice"] = "Entrenamiento adicional eliminado. El plan base no cambió."
            set_page("Hoy", selected_day)
            st.rerun()
    else:
...
# ============================================================
    ...
    if not _v74_ready:
        ...
        with st.form("profile_physiology_form"):
            ...
            sleep_hours = p4.number_input(
                ...
                step=0.5,
                help="0 = no informado. El registro diario diario sigue siendo la fuente para readiness agudo.",
            )
        ...
        if not pace_rpe_calibration_storage_ready():
            st.warning(
                "Aprendizaje personal pendiente. Ejecuta `supabase_v7_6_4_pace_rpe_calibration.sql` "
                "en Supabase y vuelve a cargar la app."
            )
        else:
            ...
            if int(_cal.get("count") or 0) == 0:
                st.info(
                    "Aún no hay sesiones suficientes para aprender tu relación entre ritmo y esfuerzo. Registra entrenamientos en **Caminadora** "
                    "con KM reales, duración y RPE. Las sesiones continuas de rodaje/recuperación/larga "
                    "alimentarán el modelo; intervalos y carreras se excluyen del ritmo medio."
                )
            else:
                ...
                for _s in (_cal.get("samples") or [])[:8]:
                    _cal_rows.append({
                        ...
                        "Ritmo": pace_string(_s.get("pace_sec_km")),
                        "RPE": f"{float(_s['rpe']):g}/10",
                        "Tipo": str(_s.get("zone") or "").title(),
                    })
                with st.expander("Ver sesiones que están enseñando al modelo", expanded=False):
    ...
    with st.expander("Comprobación interna del sistema", expanded=False):
    ...
    if ORPHAN_LOGS:
        st.warning(
            ...
            "No se incluyen en los indicadores ni gráficos."
        )
        orphan_rows = [
            {
                "Fecha": date.fromisoformat(str(x["session_date"])).strftime("%d/%m/%Y"),
                "Estado": ui_status_label(x.get("status")),
